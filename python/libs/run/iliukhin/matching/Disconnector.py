...
# 3. Сопоставление по имени и Техместу (df_supa['Диспетчерское наименование'])
df_supa['name'] = df_supa['Диспетчерское наименование']
df_supa['name_techplace'] = df_supa['name'] +'_'+ df_supa['Верхнее ТМ']

_, df_merge3, df_rs, df_supa, _ = merge_by_key_column(df_rs, df_supa, 'name')

# 4. Сопоставление по нормализованному имени и Техместу (df_supa['Диспетчерское наименование'])
