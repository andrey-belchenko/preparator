...
df_supa.loc[df_supa['Дисп_наименование'] == 'ПС 110 кВ Городская (КЭС)', 'Название'] = 'Городская-кэс'
df_supa.loc[df_supa['Дисп_наименование'] == 'ПС 110 кВ Городская (ВЭС)', 'Название'] = 'Городская-вэс'
df_supa.loc[df_supa['Дисп_наименование'] == 'ПС 110 кВ Городская (СЭС)', 'Название'] = 'Городская-сэс'
df_supa.loc[df_supa['Дисп_наименование'] == 'ПС 110 кВ Забайкальск тяговая', 'Название'] = 'ПС 110 кВ Забайкальск тяговая'
# ---
...
for uid in uids_common_now_before:
    ...
    controller = df_supa[df_supa['Техместо'] == techplace]['Дисп_наименование'].values
...
df_supa = df_supa[~df_supa['Техместо'].isin(df_union_handy['Техместо'].unique())] # СУПА несопоставленные
df_rs = df_rs[~df_rs['Uid'].isin(df_union_handy['Uid'].unique())]# RS несопоставленные
...
print('Сопоставлено', (df_union_handy.shape[0])/(df_supa.shape[0] + df_union_handy.shape[0]), 'СУПА')
print('Сопоставлено', (df_union_handy.shape[0])/(df_rs.shape[0] + df_union_handy.shape[0]), 'RS')